...
import apijson.boot.AutoTestController;
...
//    }
    ...
    public APIJSONParser<Long> setSession(HttpSession session) {
        Boolean asDBAccount = session == null ? null : (Boolean) session.getAttribute(AutoTestController.AS_DB_ACCOUNT);
        this.asDBAccount = asDBAccount != null && asDBAccount;
        if (this.asDBAccount) {
          // User user = (User) session.getAttribute(AutoTestController.USER_);
          // this.dbAccount = user.getName();
          Privacy privacy = session == null ? null : (Privacy) session.getAttribute(AutoTestController.PRIVACY_);
