...
import uigox.demo.base.BaseBottomTabActivity;
import uigox.demo.R;
...
import uigox.demo.util.TestRequestAndResponseJava;
//import uigox.demo.util.TestRequestAndResponseKt;
import uigo.x.UIAutoActivity;
import apijson.JSON;
...
public class MainTabActivity extends BaseBottomTabActivity implements OnBottomDragListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findView(R.id.tvBaseTitle, new View.OnClickListener() {
			@Override
...
//		if (SettingUtil.isOnTestMode) {
			...
			// HttpRequest.get(JSON.parseObject(JSON.toJSONString(TestRequestAndResponseJava.request()), apijson.JSONObject.class), 0, new OnHttpResponseListener() {
			// 	@Override
			...
			// });
			//
			// HttpRequest.get(JSON.parseObject(JSON.toJSONString(TestRequestAndResponseJava.smartRequest()), apijson.JSONObject.class), 0, new OnHttpResponseListener() {
			// 	@Override
			// 	public void onHttpResponse(int requestCode, String resultJson, Throwable e) {
...
//			HttpRequest.get(JSON.parseObject(JSON.toJSONString(TestRequestAndResponseKt.request()), apijson.JSONObject.class), 0, new OnHttpResponseListener() {
//				@Override
...
//			});
//
//			HttpRequest.get(JSON.parseObject(JSON.toJSONString(TestRequestAndResponseKt.smartRequest()), apijson.JSONObject.class), 0, new OnHttpResponseListener() {
//				@Override
//				public void onHttpResponse(int requestCode, String resultJson, Exception e) {
...
//		}
	...
	public void initView() {// 必须调用
		super.initView();
		exitAnim = zuo.biao.library.R.anim.bottom_push_out;

		llMainTabLeftContainer = findView(R.id.llMainTabLeftContainer);
		llMainTabRightContainer = findView(R.id.llMainTabRightContainer);
	}
