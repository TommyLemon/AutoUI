...
import static uigo.x.UIAutoListActivity.KEY_ACCOUNT;
import static uigo.x.UIAutoListActivity.KEY_ACCOUNT_ID;
import static uigo.x.UIAutoListActivity.KEY_ACCOUNT_NAME;
import static uigo.x.UIAutoListActivity.KEY_PASSWORD;
import static uigox.demo.activity_fragment.MineFragment.HTTP_LOUOUT;

import apijson.JSONResponse;
import uigo.x.UIAutoApp;
import uigo.x.UIAutoListActivity;
...
import uigox.demo.server.model.Login;
...
public class LoginActivity extends BaseActivity implements OnClickListener, OnBottomDragListener{
	 ...
	 * @return
	 */
	public static Intent createIntent(Context context, String phone, String password) {
		return createIntent(context)
				.putExtra(KEY_ACCOUNT, phone)
	...
	public void initView() {//必须调用
		exitAnim = zuo.biao.library.R.anim.bottom_push_out;//退出动画
	...
	private void onLoginSucceed() {
		intent = MainTabActivity.createIntent(UIAutoApp.getInstance().getCurrentActivity());
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
		UIAutoApp.getInstance().startActivity(intent);
	...
	public void initData() {//必须调用
		phone = getIntent().getStringExtra(KEY_ACCOUNT);
	...
	public void login(int type) {
		...
		login(type, phone, password);
	}
	public void login(int type, String phone, String password) {
		login(type, phone, password, null);
	}

	public void login(int type, String phone, String password, OnHttpResponseListener listener) {
		if (! StringUtil.isPhone(phone)) {
			showShortToast("手机号不合法，请重新输入！");
			return;
		}
		if (StringUtil.isNumber(password) == false || password.length() < 4) {
			showShortToast("密码不合法，请重新输入！");
			return;
		}
			...
			public void onHttpResponse(int requestCode, String resultJson, Throwable e) {
				...
				if (response.isSuccess() == false) {
					...
					case JSONResponse.CODE_CONDITION_ERROR:
						...
						break;
					default:
						showShortToast(zuo.biao.library.R.string.login_faild);
						break;
					}
				} else {
					user.setPhone(phone);
					...
					if (DemoApplication.getInstance().isLoggedIn()) {
						UIAutoApp.getInstance().getSharedPreferences().edit()
								.remove(KEY_ACCOUNT).putString(KEY_ACCOUNT, phone)
								.remove(KEY_PASSWORD).putString(KEY_PASSWORD, password)
								.remove(KEY_ACCOUNT_ID).putString(KEY_ACCOUNT_ID, StringUtil.get(user.getId()))
								.remove(KEY_ACCOUNT_NAME).putString(KEY_ACCOUNT_NAME, user.getName())
								.apply();
	...
	public void logout(OnHttpResponseListener listener) {
		HttpRequest.logout(HTTP_LOUOUT, listener);
		DemoApplication.getInstance().logout();

		toActivity(MainTabActivity.createIntent(UIAutoApp.getInstance().getCurrentActivity())
	...
	public void onClick(View v) {
        ...
        } else if (id == R.id.tvLoginForget) {
            showForget();
        } else if (id == R.id.tvLoginLogin) {
            login(Login.TYPE_PASSWORD);
        }
	}
