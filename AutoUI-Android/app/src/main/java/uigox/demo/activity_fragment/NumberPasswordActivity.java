...
public class NumberPasswordActivity extends BaseActivity implements OnClickListener, OnHttpResponseListener {
	...
	private void toInput(String phone, String verify) {
		...
		EditTextUtil.showKeyboard(context, etPasswordSetting);
	}
	...
	private void next() {
		if (isToSetting == false || isToConfirm) {
			...
			overridePendingTransition(zuo.biao.library.R.anim.bottom_push_in, zuo.biao.library.R.anim.hold);
		}
	}
		...
		for (int i = 0; i < tvPasswordSettingNumbers.length; i++) {
			tvPasswordSettingNumbers[i].setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					EditTextUtil.showKeyboard(context, etPasswordSetting, true);
				}
			});
		}
	...
	public void onHttpResponse(int requestCode, String resultJson, Throwable e) {
		...
		case HTTP_SET_PASSWORD:
			if (response.isSuccess()) {
				showShortToast(zuo.biao.library.R.string.change_succeed);
				saveAndExit(true);
			} else {
				showShortToast(zuo.biao.library.R.string.change_failed);
			}
			break;
		default:
			break;
	...
	public void onClick(View v) {
        //设置不响应，解决抢占焦点
        if (v.getId() == R.id.llPasswordSettingPayNumberContainer) {
        }
	}
