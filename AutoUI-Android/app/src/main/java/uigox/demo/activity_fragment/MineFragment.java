...
, OnHttpResponseListener, OnBottomDragListener, TopBarMenuCallback, OnDataChangedListener {
	...
	private void setUser(User user_) {
		...
		runUiThread(new Runnable() {
			...
			public void run() {
				if (isLoggedIn) {
					ImageUtil.loadImage(ivSettingHead, user.getHead(), ImageUtil.TYPE_ROUND_CORNER);
...
//		startActivityForResult(QRCodeScanActivity.createIntent(context, 0), REQUEST_TO_SCAN);
