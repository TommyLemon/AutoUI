 ...
 */
public class MomentAdapter extends BaseAdapter<MomentItem, MomentView> {
	...
	public MomentView createView(int position, ViewGroup parent) {
		final MomentView itemView = new MomentView(context);
		itemView.setOnDataChangedListener(new OnDataChangedListener() {
			...
			public void onDataChanged() {
				...
				if (data == null) {
					if (itemView.position >= 0 && itemView.position < list.size()) {
						list.remove(itemView.position);
						notifyDataSetChanged();
					}
				} else {
					list.set(itemView.position, data);
