...
import uigox.demo.server.model.Comment;
import uigox.demo.server.model.Moment;
import uigox.demo.server.model.Privacy;
import uigox.demo.server.model.User;
import uigox.demo.server.model.Verify;
...
public class HttpRequest {
	...
	public static void logout(int requestCode, OnHttpResponseListener listener) {
		...
		new Handler().postDelayed(new Runnable() {
			...
			@Override
			public void run() {
				HttpManager.getInstance().saveCookie(URL_BASE, null);
			}
		}, 500);
	}
	...
	public static void getUser(long id, boolean withMomentList, int requestCode, OnHttpResponseListener listener) {
		...
		if (withMomentList) {
			request.putsAll(new JSONRequest(MOMENT_
					, new JSONRequest(USER_ID, id)
					// .puts("pictureList{>", 0)
					.setColumn("pictureList").setOrder(DATE_DOWN).setHaving("json_length(pictureList)>0")
			)
			.toArray(3, 0, MOMENT_));
		}
	...
	public static void getUserList(int range, long id, com.alibaba.fastjson.JSONObject search, List<Long> idList
		...
		if (idList != null) {
			...
			uigox.demo.model.User currentUser = application.getCurrentUser();
			if (currentUser == null) {
				currentUser = new uigox.demo.model.User();
			}
			switch (range) {
	...
	public static void deleteComment(long id, long userId, int requestCode, OnHttpResponseListener listener) {
		delete(new JSONRequest(
				COMMENT_, new JSONObject(
						new Comment(id)
						).setRole(application.isCurrentUser(userId) ? AbstractVerifier.OWNER : AbstractVerifier.ADMIN)
				).setTag(COMMENT_)
				, requestCode, listener);
	}
	...
	public static void getPrivacy(long id, int requestCode, OnHttpResponseListener listener) {
		gets(new JSONRequest(
				PRIVACY_, new JSONObject(new Privacy(id)).setRole(AbstractVerifier.CIRCLE)
